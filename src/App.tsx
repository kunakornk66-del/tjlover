...
export default function App() {
  ...
  const handleSimpleLogin = async (email: string, name: string) => {
    ...
    try {
      ...
      // Extract room code from email (format: owner-[ROOMCODE]@couple.com or partner-[ROOMCODE]@couple.com)
      let roomCode = '';
      const emailParts = email.split('@')[0].split('-');
      if (emailParts.length >= 2) {
        roomCode = emailParts[1];
      } else {
        roomCode = email.split('@')[0];
      }

      const response = await appFetch('/api/simple/auth', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ yourName: name.trim(), roomCode: roomCode.trim() }),
      });
      ...
      if (response.ok) {
        ...
        // If auto-login fails (room might have been deleted/reset), clear storage and let them log in again
